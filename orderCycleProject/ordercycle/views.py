...
class PDFUploadViewSet(viewsets.ModelViewSet):
    ...
    #             try:
    #                 text = extract_text_from_first_page(pdf_path)
    #                 if "E-Kart Logistics" in text:
    #                     data = process_pdf_with_html(pdf_path)
    #                     for record in data:
    #                         FlipkarOrders.objects.create(
    #                             order_number=record["order_id"],
    #                             sku=record["sku_qty"][0]["sku"],
    #                             quantity=record["sku_qty"][0]["quantity"],
    #                             pdf_url=record["pdf_output_path"]
    #                         )
    #                     results['flipkart_processed'] += len(data)
    #                 elif "FirstCry" in text:
    #                     first_Cry_data = process_invoice_pdfs(pdf_path)
    #                     for rec in first_Cry_data:
    #                         order_num = rec["order_no"]
    #                         path = rec["pdf_path"]
    #                         for i in rec["items"]:
    #                             FirstcryOrders.objects.create(
    #                                 order_number=order_num,
    #                                 sku=i["style_code"].replace("\n",""),
    #                                 quantity=i["quantity"],
    #                                 pdf_url=path
    #                             )
    #                     results['firstcry_processed'] += len(first_Cry_data)
    #                 elif "amazon" in text:
    #                     first_Cry_data = process_invoice_pdfs(pdf_path)
    #                     for rec in first_Cry_data:
    #                         order_num = rec["order_no"]
    ...
    #                 else:
    #                     pass
    #                 # Add other platforms processing here
    #             except Exception as e:
    #                 results['errors'].append(f"Error processing {pdf.file.name}: {str(e)}")
...
class OrderCountsViewSet(ViewSet):
    ...
    def process_orders(self, request):
        """
        Process selected orders with strict batch limit enforcement
        """
        try:
            ...
            platform = request.data.get('platform', 'UNKNOWN').upper()  # Fixed: Ensure consistent case
            order_type = request.data.get('order_type', 'SINGLE').upper()  # Fixed: Ensure consistent case
            
            print(f"Processing orders: platform={platform}, order_type={order_type}, batch_limit={batch_limit}")
            print(f"Total orders received: {len(orders)}")
            
            # Convert batch_limit to integer
            try:
                batch_limit = int(batch_limit)
            except (ValueError, TypeError) as e:
                print(f"Error converting batch_limit to int: {e}")
                batch_limit = 50
            
            # Validate batch_limit
            if batch_limit <= 0:
                batch_limit = 50
            
            # STRICT ENFORCEMENT: Always limit to batch_limit number of orders
            orders_to_process = orders[:batch_limit]
            
            print(f"Strictly limiting to first {batch_limit} orders: {len(orders_to_process)} orders will be processed")
            ...
            if len(orders_to_process) > 0:
                ...
                picklist = Picklist.objects.create(
                    ...
                    quantity=len(orders_to_process),
                    platform=platform,
                    status='CREATED'
                )
                ...
                db_order_type = 'Single' if order_type == 'SINGLE' else 'Multiple'  # Fixed: consistent order type mapping
                
                # Fetch orders one by one to maintain exact order and strict limit
                ...
                # If no orders were actually processed, delete the picklist
                ...
                # Update picklist quantity to match actual processed count
                picklist.quantity = len(processed_orders)
                picklist.save()
                
                return Response({
                    'status': 'success',
                    'message': f'Processed {len(processed_orders)} {platform} orders',  # Fixed: Show correct platform
